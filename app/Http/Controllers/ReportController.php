...
class ReportController extends Controller
{
    ...
    public function DashboardReport()
    {
        ...
        // Sales Report
        $dailysales = floor(DB::table('tbl_order_detail')->whereDate('created_at', today())->sum('OD_Price'));
        $weeklysales = floor(DB::table('tbl_order_detail')->whereBetween('created_at', [$startOfWeek, $endOfWeek])->sum('OD_Price'));
        $monthlysales = floor(DB::table('tbl_order_detail')->whereBetween('created_at', [$startOfGregorianMonth, $endOfGregorianMonth])->sum('OD_Price'));
        $totalsales = floor(DB::table('tbl_order_detail')->sum('OD_Price'));
        
        // Expenses Report
        $dailyExpenses = floor(DB::table('tbl__expenses')->whereDate('created_at', today())->sum('E_Amount'));
        $weeklyExpenses = floor(DB::table('tbl__expenses')->whereBetween('created_at', [$startOfWeek, $endOfWeek])->sum('E_Amount'));
        $monthlyExpenses = floor(DB::table('tbl__expenses')->whereBetween('created_at', [$startOfGregorianMonth, $endOfGregorianMonth])->sum('E_Amount'));
        $totalExpenses = floor(DB::table('tbl__expenses')->sum('E_Amount'));

        // Storage Report
        $dailystorage = floor(DB::table('tbl_storage__detail')
            ->whereDate('created_at', today())
            ->where('S_status', 'In')
            ->selectRaw('SUM(S_Unit * S_Price) as total')
            ->value('total'));

        $weeklystorage = floor(DB::table('tbl_storage__detail')
            ->whereBetween('created_at', [$startOfWeek, $endOfWeek])
            ->where('S_status', 'In')
            ->selectRaw('SUM(S_Unit * S_Price) as total')
            ->value('total'));

        $monthlystorage = floor(DB::table('tbl_storage__detail')
            ->whereBetween('created_at', [$startOfGregorianMonth, $endOfGregorianMonth])
            ->where('S_status', 'In')
            ->selectRaw('SUM(S_Unit * S_Price) as total')
            ->value('total'));

        $totalstorage = floor(DB::table('tbl_storage__detail')
            ->where('S_status', 'In')
            ->selectRaw('SUM(S_Unit * S_Price) as total')
            ->value('total'));
